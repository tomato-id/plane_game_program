...
class PlaneGame(object):
    ...
    def __check_collide(self):
        ...
        for x in [self.enemy0_group, self.enemy1_group, self.enemy2_group]:
            shot_enemy = pygame.sprite.groupcollide(self.hero.bullets, x, False, False)
            if len(shot_enemy) > 0:
                for enemys in shot_enemy:
                    ...
                    for enemy in enemy_list:
                        self.enemy = enemy
                        self.enemy.HP -= 1
                for bullet in shot_enemy.keys():
                    if not self.enemy.isboom:
                        bullet.kill()

        # 敌机撞毁英雄
        for y in [self.enemy0_group, self.enemy1_group, self.enemy2_group]:
            shot_hero = pygame.sprite.spritecollide(self.hero, y, False)
            if len(shot_hero) > 0:
                for hit_hero in shot_hero:
                    if self.hero.isboom is False:
                        hit_hero.HP = 0
    ...
    def __update_sprite(self):
        ...
        # 敌机精灵
        enemy_group = [self.enemy0_group, self.enemy1_group, self.enemy2_group]
        for m in enemy_group:
            copy_m = m.copy()
            for n in m:
                if n.isboom:
                    copy_m.remove(n)
                    kill_enemy = n.boom(self.screen)
                    if kill_enemy:
                        n.kill()
            copy_m.update()
            copy_m.draw(self.screen)

        # if self.enemy:
        #     kill_enemy = self.enemy.boom(self.screen)
        #     if kill_enemy:
        #         self.enemy.kill()
